...
useEffect(() => {
      ...
      setProjects((prev: Project[]) =>
        prev.map((p) =>
          p.id === update.projectId
            ? { ...p, ...(update.data as Partial<Project>) } // هنا بنأكد إنه جزئي
            : p
        )
      );
    }
  ...
  const cancelEditingProgress = () => setEditingProgress(null);
          ...
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              {["name", "status", "startDate", "endDate", "progress", "budget"].map((col) => (
                <th
                  key={col}
                  onClick={() => handleSort(col as keyof Project)}
                  className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition"
                >
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {editingProgress?.id === project.id ? (
                      <div className="flex items-center gap-1">
                        ...
                        />
                        <button
                          onClick={() => handleProgressEdit(project.id, editingProgress.value)}
                          className="px-2 py-1 bg-green-500 text-white text-xs rounded hover:bg-green-600"
                          title="Save"
                        >
                          ✓
                      ...
                      <button
                        onClick={() => startEditingProgress(project.id, project.progress)}
                        className="text-sm text-indigo-600 hover:text-indigo-800 font-medium"
                        disabled={role !== "Admin" && role !== "ProjectManager"}
                      >
                        {project.progress}%
